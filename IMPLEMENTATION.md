...
### Fase 8 · Síntesis final con Inngest (4 horas) — ✅ CERRADA 2026-05-14 (storage Blob + notif email + admin viewer)
- [x] **Inngest function `sintetizarSesion`** — `lib/inngest/functions/sintetizarSesion.ts` con `triggers: [{ event: 'sesion/lista_para_sintesis' }]`, `retries: 4`, 3 step.run aislados (`procesar-sintesis-final` + `generar-pdf` + `notificar-admin`). Wirado vía `dispatchSesionListaParaSintesis` (lib/motor/review.ts:579).
...
- [x] **Storage del PDF** (Vercel Blob) — `@vercel/blob@2.3.3` instalado (2026-05-14). `lib/storage/blob.ts` cablea `uploadPdfToBlob(buffer, pathname)` con gating por `BLOB_READ_WRITE_TOKEN` + dynamic import. Step `generar-pdf` (sintetizarSesion.ts:121) sube el PDF y persiste `pdf_url` en `perfil_decision_final` via UPDATE. Migración 0007 aplicada a `vertice-mvp/main`. Degradación grácil: sin token, `uploadPdfToBlob` retorna null y el flujo sigue sin URL persistida.
- [x] **Notificación admin** (Resend email) — 2026-05-14. `sendSintesisCompleta` en `lib/email/resend.ts` con Spanish MX: subject "Vértice · Síntesis lista: {razon_social}", body con link a `/admin/sesiones/{id}` + métricas (completitud %, confianza %, ratio cajas) + link al PDF si `pdf_url` existe. Step `notificar-admin` (sintetizarSesion.ts:181) lee `ADMIN_EMAILS` env (comma-separated, parseado via `parseAdminEmails`), llama Resend, errores non-fatal. 13 unit tests cubren parsing + payload shape + XSS escape + degradación sin PDF.
- [x] **Admin viewer PDF link** — `app/admin/sesiones/[id]/page.tsx` renderea botón pill ink "Descargar PDF de síntesis" cuando `perfilSesion.pdf_url` existe (con `target=_blank rel=noopener noreferrer`), fallback "PDF aún no generado o storage no configurado (BLOB_READ_WRITE_TOKEN pendiente)" cuando no.
- [x] **Smoke E2E `scripts/smoke_fase8.ts`** — 7/7 verde 2026-05-14. Etapas: find-institucion → upsert-sesion → build-perfil → generar-pdf (482KB en 3.3s) → upload-blob (skipped sin token, degradación grácil) → notif-email (Resend OK) → admin-viewer. Validación visual en Chrome via Playwright (token emergency mode): vista admin renderea métricas + fallback PDF correctamente; con `pdf_url` seteada manualmente el botón aparece.

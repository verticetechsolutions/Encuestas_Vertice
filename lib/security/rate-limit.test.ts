...
describe('checkRateLimit', () => {
  ...
  it('permite hasta `capacity` requests consecutivos', async () => {
    const cfg = { capacity: 3, refillPerSecond: 1 };
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true);
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true);
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true);
    expect((await checkRateLimit('k', cfg)).allowed).toBe(false);
  });

  it('reporta remaining decreciente y retryAfter cuando se agota', async () => {
    const cfg = { capacity: 2, refillPerSecond: 0.5 }; // 0.5 token/s = 1 cada 2s
    const a = await checkRateLimit('k', cfg);
    expect(a.allowed).toBe(true);
    expect(a.remaining).toBe(1);
    const b = await checkRateLimit('k', cfg);
    expect(b.allowed).toBe(true);
    expect(b.remaining).toBe(0);
    const c = await checkRateLimit('k', cfg);
    expect(c.allowed).toBe(false);
    expect(c.remaining).toBe(0);
    expect(c.retryAfterSeconds).toBeGreaterThanOrEqual(1);
  });

  it('refilla tokens con el paso del tiempo', async () => {
  ...
  it('aísla buckets por key', async () => {
    ...
    expect((await checkRateLimit('b', cfg)).allowed).toBe(false);
  });

  it('cap de capacity al refillar (no excede)', async () => {
    const cfg = { capacity: 3, refillPerSecond: 100 };
    await checkRateLimit('k', cfg); // consume 1 → 2 left
    vi.advanceTimersByTime(60_000); // refill grande → debería capear a 3
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true); // 1 consumido → 2 left
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true); // 1 consumido → 1 left
    expect((await checkRateLimit('k', cfg)).allowed).toBe(true); // 1 consumido → 0 left
    expect((await checkRateLimit('k', cfg)).allowed).toBe(false); // empty
  });
